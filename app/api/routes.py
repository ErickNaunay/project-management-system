...
def list_projects(service: ProjectService = Depends(get_project_service)):
    return [ ProjectOut(id = project.id, name = project.name) for project in service.list()]
...
def uodate_tasks(task_id: str, body: TaskUpdate, service: TaskService = Depends(get_task_service)):
    try:
