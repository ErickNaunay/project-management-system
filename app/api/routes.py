...
def create_task(project_id: str, body: TaskCreate, service: TaskService = Depends(get_task_service)):
    try:
        ...
        return TaskOut(id=task.id, project_id=task.project_id, 
                       title=task.title, status=task.status, 
                       due_date=task.due_date, priority_score=task.priority_score)
    except Exception as e:
        raise to_http(e)
...
def get_tasks(project_id: str, service: TaskService = Depends(get_task_service)):
    try:
        ...
        return [TaskOut(id=t.id, project_id=t.project_id, title=t.title, 
                        status=t.status, due_date=t.due_date, 
                        priority_score=t.priority_score) for t in tasks]
    except Exception as e:
        raise to_http(e)
